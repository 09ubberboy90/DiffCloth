...
double AttachmentSpring::k_stiff = 10000.0; // TODO: WARNING: change back to 10000

double AttachmentSpring::evaluateEnergy(const VecXd& x_new)
{
    energyBuffer = (0.5 * k_stiff * (p1_vec3(x_new) - fixedPointPos()).squaredNorm());
    return energyBuffer;
}
...
void AttachmentSpring::projectBackward(const VecXd& x_vec, TripleVector& triplets)
{
...
void AttachmentSpring::addConstraint(std::vector<Triplet>& tri, int& c_idx, bool withWeight)
{
    if (withWeight)
    {
        this->c_idx = c_idx;

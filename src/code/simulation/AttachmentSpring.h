...
public:
    ...
    AttachmentSpring(int p1_idx, std::vector<Particle>* pArr, int pfixed_idx, std::vector<FixedPoint>* pFixedArr)
        : Constraint(
              Constraint::ConstraintType::CONSTRAINT_ATTACHMENT, 3), p1_idx(p1_idx), pfixed_idx(pfixed_idx), pArr(pArr),
          pFixedArr(pFixedArr),
          sqrtConstraintWeight(std::sqrt(AttachmentSpring::k_stiff))
    {
    }

    AttachmentSpring(const AttachmentSpring& other) : p1_idx(other.p1_idx), pfixed_idx(other.pfixed_idx),
                                                      sqrtConstraintWeight(other.sqrtConstraintWeight),
                                                      pArr(other.pArr), pFixedArr(other.pFixedArr)
    {
    }
    ...
    {
        //      std::printf("fixedPos: %.3f %.3f %.3f\n", pFixed()->pos[0], pFixed()->pos[1], pFixed()->pos[2] );
        //      std::printf("p1: %.3f %.3f %.3f\n", p1()->pos[0], p1()->pos[1], p1()->pos[2] );
        return pFixed()->pos;
    }

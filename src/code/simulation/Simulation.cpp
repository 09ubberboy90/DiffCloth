...
double* Simulation::k_stiff_arr[Constraint::CONSTRAINT_NUM] = {
    &Spring::k_stiff, &AttachmentSpring::k_stiff,
                                    ...
                                    const VecXd& v_prim_new) const
{
    ...
    for (int i = 0; i < primitives.size(); i++)
    {
        ...
        if (p->isEnabled)
        {
            ...
            if (p->isInContact(primPos, pos + v_in * sceneConfig.timeStep * 0.5, v_in, info.normal, info.dist,
                               info.v_out))
            {
                info.collides = true;
                info.primitiveId = i;
...
#pragma omp parallel for if (OPENMP_ENABLED)
        for (int i = 0; i < particles.size(); i++)
        {
            ...
            fullCollisionArr[i] = isInContactWithObstacle(x_n.segment(p.idx * 3, 3), v.segment(p.idx * 3, 3), x_prim,
                                                          v_prim);
        }
  ...
  for (int i = 0; i < particles.size(); i++) {
      ...
      std::printf("\n");
            ...
            for (int particleId : frontier)
            {
                ...
                for (int otherId : selfCollisionMap[particleId])
                {
                    ...
                    // remove from datastructure
                    removeInfoFromDataStructures(particleId, otherId, infoIdx, info, selfCollisionMap,
                                                 selfCollisionTable);
    ...
    std::printf("Finished current exploitation\n");
#endif
        if (processedSelfCollisions != selfCollisions.size())
        {
            // sorting not finished. Explore new frontier
#ifdef  DEBUG_SELFCOLLISION
      std::printf("Sorting not finished. Exploring new frontier\n");
            ...
            if (frontier.empty())
            {
                // there is a self-collision loop in this case
                                                                     ...
                                                                     bool calculateDensityGradient = false) const
{
    ...
    if (contactEnabled)
    {
        ...
        if (selfcollisionEnabled)
        {
            ...
            for (const std::vector<SelfCollisionInformation>& selfInfos : infos.second)
            {
                // has to use second, because 1. layer matters 2. info.d and info.r is only updated in second
                layerCount++;
...
Vec3d Simulation::calcualteDryFrictionForce(const Vec3d& n, const Vec3d& f_i, double mu, CollisionType& type) const
{
    ...
    {
        r_i += -f_N; // r_N += -f_N
        if (dT_norm <= mu * std::abs(sign_dist))
        {
            // stick
            r_i += -f_T;
            if (printCases)
...
Vec3d Simulation::calculatedri_dmu(const Vec3d& n, const Vec3d& f_i, double mu) const
{
    ...
    {
        if (dT_norm > mu * std::abs(dN_signed_dist))
        {
            //slip
    ...
    case TrajectoryConfigs::FIXED_POINT_TRAJECTORY:
        {
            ...
            //                                                                                 1]
            //                                                                .segment(fixedPointIdx * 3, 3);
            // }
            auto currPos = sysMat[currentSysmatId].fixedPoints[0].pos;
            currPos[1] += 0.1;
            sysMat[currentSysmatId].fixedPoints[0].pos = currPos;
            break;
        }
    ...
    case TrajectoryConfigs::PER_STEP_TRAJECTORY:
        {
            ...
            // // }
            Vec3d posLast = sysMat[currentSysmatId].fixedPoints[0].pos;
            posLast(1) += 0.005;
            std::cout << "timestep: " << t << std::endl;
            std::cout << posLast(0) << ", " << posLast(1) << ", " << posLast(2) << std::endl;
            std::cout << sysMat[currentSysmatId].fixedPoints[1].pos(0) << ", " << sysMat[currentSysmatId].fixedPoints[1]
                .pos(1) << ", " << sysMat[currentSysmatId].fixedPoints[1].pos(2) << std::endl;
            rlFixedPointPos = VecXd(
                posLast.size() + sysMat[currentSysmatId].fixedPoints[1].pos.size() + sysMat[currentSysmatId].fixedPoints
                [2].pos.size() + sysMat[currentSysmatId].fixedPoints[3].pos.size());
            rlFixedPointPos << posLast, sysMat[currentSysmatId].fixedPoints[1].pos, sysMat[currentSysmatId].fixedPoints[
                2].pos, sysMat[currentSysmatId].fixedPoints[3].pos;
...
void Simulation::stepNN(int idx, const VecXd& x, const VecXd& v, const VecXd& fixedPointPos)
{
    ...
    {
        Logging::logWarning(
            "require" + std::to_string(requiredDofs) + " fixed point dofs but input fixed point dof is " +
            std::to_string(fixedPointDofs) + "\n");
    }
...
void Simulation::step()
{
  ...
  std::vector<std::pair<VecXd, VecXd>> x_newAndErrors;
    ...
    for (int i = 0; i < primitives.size(); i++)
    {
        ...
        delta_v_primitives.segment(3 * i, 3) = f_primitives.segment(3 * i, 3) / primitives[i]->mass * sceneConfig.
            timeStep;
...
#pragma omp parallel for if (OPENMP_ENABLED)
            for (int i = 0; i < sysMat[currentSysmatId].constraints.size(); i++)
            {
                ...
                projections_pertype[c->constraintType].segment(c->c_weightless_idx, c->constraintNum) = projections.
                    segment(
...
#pragma omp parallel for if (OPENMP_ENABLED)
            ...
            b = (sysMat[currentSysmatId].A_t * projections) * (sceneConfig.timeStep * sceneConfig.timeStep) +
                M_times_sn;
            timeSteptimer.toc();
          ...
          x_newAndErrors.emplace_back(std::make_pair(x_new, P * x_new - (b_tilde + r)));
            ...
            if (converged)
            {
                ...
                (forwardRecords.empty()
                     ? 0
                     : forwardRecords[forwardRecords.size() -
                         1].cumulateIter);
                timeSteptimer.toc();
                break;
            }
...
VecXd Simulation::solveDirect(VecXd& dL_dxnew, double t_2, SpMat& dproj_dxnew_t, SystemMatrix& currentSysMat,
                              SpMat& dr_df_plusI_t, SpMat& dr_df_t)
{
...
#pragma omp parallel for if (OPENMP_ENABLED)
...

        {
            ...
            else
            {
                ...
                for (int iterIdx = 0; iterIdx < MAX_ITER_NUM; iterIdx++)
                {
                    ...
                    bool converged =
                        (std::abs((u_star - u_star_prev).norm() / (particles.size() * 1.0))) <
    ...
    if (taskInfo.dL_dcontrolPoints && (!currentSysMat.controlPointSplines.empty()))
    {
        ...
        // 3m x s
        SpMat rhs_xfixed = sceneConfig.timeStep * sceneConfig.timeStep * dr_df_plusI * sysMat[sysMatId].A_t_dp_dxfixed;
        // 3m x s
    ...
    {
        ret.dL_dconstantForceField = gradient_new.dL_dconstantForceField + sceneConfig.timeStep * sceneConfig.timeStep *
...
void Simulation::createBendingConstraints()
{
    ...
    for (int triIdx = 0; triIdx < mesh.size(); triIdx++)
    {
        ...
        for (int v1Idx = 0; v1Idx < 3; v1Idx++)
        {
            ...
            {
                int minIdx = std::min(t.idxArr[v1Idx], t.idxArr[v2Idx]), maxIdx = std::max(
                        t.idxArr[v1Idx], t.idxArr[v2Idx]);
...
void Simulation::createClothMeshFromModel(std::vector<Vec3d>& particleIn, std::vector<Vec3i>& meshIn)
{
    ...
    double scale = sceneConfig.fabric.keepOriginalScalePoint
                       ? 1.0
                       : std::max(std::max(dim[0], dim[1]), dim[2]) /
        ...
        case LEFT_RIGHT_CORNERS_2:
            {
                ...
                Vec3d lowerRightGoalPoint = Vec3d(restShapeMaxDim[0], restShapeMinDim[1],
                                                  (restShapeMinDim[2] + restShapeMaxDim[2]) / 2.0);
                std::vector<std::pair<Vec3d, Particle*>> goalPoints = {
                    {upperLeftGoalPoint, &particles[0]},
        ...
        case LEFT_RIGHT_CORNERS_2:
            {
                ...
                std::pair<int, int> p4XY = std::make_pair(sceneConfig.fabric.gridNumY - 1,
                                                          sceneConfig.fabric.gridNumX - 1);
                gridXYs = {p1XY, p2XY};
                break;
            }
            ...
            {
                {
                    ...
                    Vec3d point0XEnd = getInitParticlePos(sceneConfig.fabric.gridNumY - 1,
                                                          sceneConfig.fabric.gridNumX - 1);
                    double yUp = 8;
                    ...
                    if (sceneConfig.trajectory != CORNERS_1_WEARHAT)
                    {
                        ...
                        sysMat[0].controlPointSplines.emplace_back(sysMat[0].fixedPoints[1].pos_rest, point0XEnd, yUp,
                                                                   1);
                    }
                }
                break;
    ...
    case AttachmentConfigs::CUSTOM_ARRAY:
        {
            ...
            for (int setIdx = 0; setIdx < sceneConfig.customAttachmentVertexIdx.size(); setIdx++)
            {
                ...
                for (int idx = 0; idx < config.second.size(); idx++)
                {
                    ...
                    sysMat[setIdx].attachments[idx] = AttachmentSpring(p.idx, &particles, idx,
                                                                       &(sysMat[setIdx].fixedPoints));
                }
...
void Simulation::createClothMesh()
{
    ...
    if (sceneConfig.fabric.isModel)
    {
        ...
        if (sceneConfig.fabric.custominitPos)
        {
            ...
            {
                std::printf(
                    "WARNING: Custom Mesh Init Pos loading from %s error: mesh vertex num is %d but loaded %d vertex\n",
                         ...
                         Vec3d center, bool runBackward)
{
    ...
    msSystem->myConfig = {
        .x0 = msSystem->forwardRecords[0].x, .v0 = msSystem->forwardRecords[0].v, .k_stiff = Triangle::k_stiff
    };
                                  ...
                                  Simulation::ParamInfo& param)
{
    ...
    if (taskConfiguration.dL_dx0)
    {
        // for rest shape param, set it after all resets have been finished
                                       ...
                                       const std::function<void(const std::string&)>& setTextBoxCB)
{
    ...
    int idxSpace = 200;
    // since full backward records require FORWARD_STEPS^2 steps simulation, we calculate records with spacing instead.
    int startSepIdx = 0;
        ...
        for (int setidx = guess.controlPointSplines.size() - 1; setidx < guess.controlPointSplines.size(); setidx++)
        {
            ...
            for (int splineIdx = 0; splineIdx < spline_center.size(); splineIdx++)
            {
                ...
                for (int dim = 0; dim < splineParameterNumber; dim++)
                {
                    ...
                    calculateFiniteDiffLossArr(paramPlus, paramMinus, delta, centerForward, L, taskConfiguration,
                                               lossType,
                                               lossInfo,
                                               FORWARD_STEPS, setTextBoxCB);
    ...
    for (int i = 0; i < forwardRecords.size(); i++)
    {
        ...
        for (int fixedPointId = 0; fixedPointId < forwardRecords[i].x_fixedpoints.rows() / 3; fixedPointId++)
        {
            ...
            MeshFileHandler::saveOBJFile(
                subFolder + std::to_string(primitives.size() + fixedPointId + 1) + "-" + "CLIP",
                primPos, primMesh);
        }
                            ...
                            const std::function<void(const std::string&)>& setTextBoxCB, bool skipForward)
{
    ...
    std::printf("forwardConverge set from %.10f to %.10f\n", Simulation::forwardConvergenceThreshold,
                taskConfiguration.forwardAccuracyLevel);
                                  ...
                                  Simulation::BackwardTaskInformation taskInfo, bool writePerf)
{
    ...
    {
        std::string experimentName = sceneConfig.name + "-randseed-" + std::to_string(taskInfo.srandSeed) + "-" +
            currentTimestampToStr() + "-forwardThresh-" +
                                        ...
                                        std::string subFolder, int prevWrriten, int newWritten)
{
    ...
    {
        MeshFileHandler::saveOBJFile(subFolder + "0-CLOTH_forwardlastframe_iter_" + std::to_string(i),
                                     records[i].second.x, mesh);
        if (i == 0)
            for (int primId = 0; primId < primitives.size(); primId++)
            {
                Primitive* prim = primitives[primId];

...
public:
    ...
    Spline(Vec3d p0, Vec3d p1, double yUp, int pFixed, double startFraction = 0.0,
           double endFraction = 1.0) : pFixed(pFixed)
    {
        Segment seg;
        seg.p0 = p0;
        seg.p1 = p1;
    ...
    void addSegment(Vec3d p1, int yUp, double startFraction, double endFraction)
    {
        ...
        {
            std::printf(
                "WARNING: calling addSegment but spline is empty at the moment, please init spline with a segment\n");
            return;
        }
        Segment seg;
    ...
    static double p0Constant(double t, int order = 0)
    {
        // t should be the actual spline percentage, so t=0 will map to splineStart and t=1 will map to splineEnd
        double t_3 = t * t * t;
        double t_2 = t * t;
    ...
    MatXd dxfixed_dcontrolPoints(double t)
    {
        // deval(x)_dcontrolPoints
        // m0,p1,m1
        int paramNum = getParameterNumber();
        int singleSplineParamNum = getParameterNumber(type);
        ...
        case ENDPOINT_AND_UP:
            {
                retSingle.block<3, 3>(0, 0) = (p1Constant(t) + m0Constant(t) + m1Constant(t)) * Mat3x3d::Identity();
                //p1
    ...
    void updateControlPoints(const VecXd& step)
    {
        // (p1, m0, m1)

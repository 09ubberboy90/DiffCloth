...
public:
    ...
    Spring(int p1_idx, int p2_idx, std::vector<Particle>& pArr, double ks, double L) : Constraint(
            Constraint::ConstraintType::CONSTRAINT_SPRING_STRETCH, 3), p1_idx(p1_idx), p2_idx(p2_idx), pArr(pArr),
        k_s(ks), l0(L),
        sqrtConstraintWeight(std::sqrt(
            Spring::k_stiff))
    {
    }

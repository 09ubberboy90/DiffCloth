...
Vec12d TriangleBending::bendingForce(const VecXd& x_new)
{
   ...
   for (int i = 0; i < 4; i++) {
     Vec3d x_i = x_new.segment(idx_arr[i] * 3, 3);
     double w_i = weightVert[i];
     gradE.segment(3 * i, 3) += w_constraint * (sumwx - proj).transpose() * (w_i * Mat3x3d::Identity() - grad_proj.block<3, 3>(0, 3 * i) );
   }
   */
...
TriangleBending::TriangleBending(int p0_idx, int p1_idx, int p2_idx, int p3_idx, std::vector<Particle>& pArr) :
    Constraint(Constraint::ConstraintType::CONSTRAINT_TRIANGLE_BENDING, 3), p0_idx(p0_idx), p1_idx(p1_idx),
    p2_idx(p2_idx), p3_idx(p3_idx),
    pArr(pArr)
{
    idx_arr.clear();

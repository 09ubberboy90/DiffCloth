...
void Spring::addConstraint(std::vector<Triplet>& tri, int& c_idx, bool withWeight)
{
    ...
    {
        std::printf(
            "This is a spring constraint, which is deprecated in favor to triangle constraint. Should not be included in the backward gradient calculation regarding to stiffness parameter\n");
        exit(0);
    }
    this->c_idx = c_idx;

...
public:
    ...
    typedef std::pair<std::vector<PrimitiveCollisionInformation>, std::vector<SelfCollisionInformation>>
    collisionInfoPair;
    ...
    struct TaskSolveStatistics
    {
        ...
        std::vector<std::pair<ParamInfo, ForwardInformation>> completeForwardLog;
        // For each simulation, only log the last frame of forward sim
        std::vector<std::pair<ParamInfo, BackwardInformation>> completeBackwardLog;
        // For each backprop, only log the first frame of backprop
    ...
    struct SceneConfiguration
    {
        ...
        std::vector<std::pair<double, std::vector<int>>> customAttachmentVertexIdx;
        // outside loop: different sets; each element: startFrame x vIdx
    ...
    static volatile bool gravityEnabled, windEnabled, staticEnabled, contactEnabled, selfcollisionEnabled,
                         enableConstantForcefield;
    ...
    struct SystemMatrix
    {
        // All these matrices are constraint weight dependent, so if constraint weight change, these matrices change
    ...
    VecXd x_n, v_n, gravity_n, external_force_field;
    // velocity and position from previous solved state / beginning of current timestep
    ...
    std::vector<std::pair<std::vector<ForwardInformation>, std::vector<BackwardInformation>>>
    ...
    void resetForwardRecordsFromFolder(std::string subFolder)
    {
        ...
        Logging::logNormal(
            "Loaded " + std::to_string(frameFileNames.size()) + " cloth frame files from folder " + fullFolderPath +
            "\n");
        ...
        for (int i = 0; i < frameFileNames.size(); i++)
        {
            ...
            for (Primitive* p : primitives)
            {
                ...
                {
                    // TODO: this is a hacky way to reset primitive trees with <= 2 levels; should be changed to recursion
    ...
    static std::string backwrdInfoAndGradToString(const Simulation::BackwardTaskInformation& taskInfo,

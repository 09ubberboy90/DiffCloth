...
public:
    ...
    Primitive(PrimitiveType t, const Vec3d& c, bool isCollection, Vec3d color) : mu(0), type(t), center(c),
        centerInit(c),
        mass(30),
        velocity(0, 0, 0),
        velocityInit(0, 0, 0),
    ...
    void updateToPose(std::vector<Triangle>& cumulativeMesh, VecXd& cumulativePos, std::vector<Triangle>& mesh,
                      std::vector<Particle>& points, Vec3d center)
    {
...
public:
    double radius;
    double length;
    Vec3d axis, parentAxis, globalAxis;
    // axis is relative to parentAxis,  globalAxis is the actual direciton of the joint
    Eigen::Transform<double, 3, Eigen::Affine> rotationFromParent, globalRotation;
    Capsule* parent;
    Primitive* rootPrimitiveContainer;
    bool isRoot;
    Capsule(Vec3d anchorCenter, double radius, double length, Vec3d parentAxis, Vec3d axis, Capsule* parent = nullptr,
            bool isRoot = true,

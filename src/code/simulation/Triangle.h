...
public:
    ...
    Eigen::Matrix<double, 6, 9> dF_dx; // gradient of deformation gradient wrt position

    Triangle(int p0_idx, int p1_idx, int p2_idx, std::vector<Particle>& pArr, bool checkArea = false);
    ...
    Vec3i getIdxVector()
    {
        return Vec3i(p0_idx, p1_idx, p2_idx);
    }
    ...
    Vec3d p0_vec3(const VecXd& x) const
    {
        return x.segment(p0()->idx * 3, 3);
    }

    Vec3d p1_vec3(const VecXd& x) const
    {
        return x.segment(p1()->idx * 3, 3);
    }

    Vec3d p2_vec3(const VecXd& x) const
    {
        return x.segment(p2()->idx * 3, 3);
    }
    ...
    Mat3x3d skew(const Vec3d& a) const
    {
        // a x b = skew(a) x b
        std::vector<double> skewElements = {
            0, -a[2], a[1],
            a[2], 0, -a[0],
            -a[1], a[0], 0
        };
    ...
    Vec9d dArea_dxnew(const VecXd& x) const
    {
        ...
        Vec9d dout_dx0x1x2 = (dout_dv1xv2.transpose() * dv1xv2_v1v2 * dv1v2_dx0x1x2).transpose();
    ...
    void addConstraint(std::vector<Triplet>& tri, int& c_idx, bool withWeight) override;
    ...
    //    std::pair<Eigen::Matrix<double, 9, 9>, Vec9d> stretchingForceAndJacobian() const;


    std::pair<Mat6x9d, Mat3x2d> forwardBackwardCheck(const VecXd& x_vec) const;

#ifdef USE_DEBUG
    std::pair<std::vector<Vec9d>, Mat9x9d> stretchingForceAndJacobianCheck() const;
#endif
